...
func make_initial_groups():
	...
	for i_race in races.size() * 3:
		var group_node: SelectableGroup = preload("res://unit/group.tscn").instantiate()
		var group_angle = rand_start_angle + (PI * 2 / (races.size() * 3)) * i_race
		print("group_angle %f" % group_angle)
		group_node.global_position = Vector2.RIGHT.rotated(group_angle) * randf_range(800, 2600)
		group_node.name = race_names[i_race % 3]
...
func _on_game_player_changed(player: Player):
	...
	for group in get_children():
		var group_nodes = get_tree().get_nodes_in_group(group.name)
		if group_nodes[0].race == player.race:
			for node: Unit in group_nodes:
